...
const fetchUsers = async (page = 1, size = 10, sortBy = "lastName", direction = "asc") => {
      ...
      <div className="p-1" style={{ position: 'relative', zIndex: 10 }}>
        {(isEditing || isCreating) && editUser && (
          <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-md mt-4" style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20 }}>
            <h2 className="text-lg font-semibold mb-4">{isCreating ? "Create User" : "Edit User"}</h2>
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="px-4 py-2 border-b border-gray-200">{user.status ? 'Active' : 'Inactive'}</td>
                  <td className="px-4 py-2 border-b border-gray-200">{user.image}</td>
